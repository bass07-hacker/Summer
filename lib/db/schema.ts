...
export const trips = pgTable('trips', { id: text('id').primaryKey(), userId: text('userId').notNull(), name: text('name').notNull(), destination: text('destination').notNull(), budget: numeric('budget').notNull().default('0'), status: text('status').notNull().default('planning'), createdAt: timestamp('created_at').notNull().defaultNow() })
